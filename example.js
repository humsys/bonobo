///**Chatterbase**\\\\Today we are going to build a little chatroom with _**soft automation**_.\\\\Most things that are accomplished by social software could be done by hand just using messaging—you could run a book club, organize 1000 workers in a construction site, or run a dating site just by using something like facebook messenger. You’d have to collect tasks and photos and profiles and everything by hand and forward them around, and so on.\\\\This would be awful. The tyrannies of coordination via messaging include:\\- **Endless checking,** to move things along,//- **Can’t get an overview**//- Interactions are limited to **short term utterances **rather than** long-term plans**\\And because there's no explicit process that means:\\- You're always **starting from scratch**,//- No **clarity about ongoing roles**,//- There are **covert, unintentional power dynamics**,//- There's nowhere to encode **organizational wisdom about process**\\But the automation of our lives—via bots or other social software—is awful in it’s own way. The tyrannies of bots include:\\- **Rigidity**. It's difficult for participants to do something different than what the script assumes, or post data that doesn't fit schemas.//- _**Impersonal Authority**_. People report to the script or to dashboards, rather than to one another. The script becomes an awful authority. No one feels responsible for the broader goal.//- _**Aggressive Mediation**_. These systems assume everything will be done through their screen-based interfaces. Simple social actions can't happen naturally through convesation in-person because they won't advance the script.//- _**Esoterism**_. Users with better ideas often can't change the way the automation works, or suggest alternatives, or even see the overall gist of the automation.\\The goal of soft automation is to be free both from the _tyrannies of messaging_ and from _the tyrannies of conventional automation_.\\\\\\**An approach to software**\\\\It's likely that the approach here can applied very broadly: most social software is a kind of automated conversation between multiple parties.\\\\Usually the conversational nature of software is obscured because the participants are filling out forms or using widgets instead of typing sentences.\\\\But it's likely that most software could be decomposed cleanly into three different kinds of specifications:\\- declarative views on data//- numerical algorithms//- routing information about which information to collect, from who, and where to put it\\If this is true, then an approach that works for soft automation in chat could—if augmented with other systems for declaring views on data and for making pure function algorithms—show how to make soft automation everywhere.\\\\\\**Soft automation in chat**\\\\So here's what it means for chat: with soft automation, we can start a thread in the chatroom that has a script associated with it, and the script says how to guide people along through a set of roles and expectations, maybe collecting some data, using notifications.\\\\Our goal is to make it easier for ordinary people to describe a set of social interactions they want automatically facilitated.\\\\To avoid the pitfalls above, our engine must support:\\- _**Discretion**_. It should be easy for each participant to do something different than what the script is suggesting to them. We will see if we can handle people accepting roles and then shedding them later, posting data that doesn't fit schemas, and the like.//- _**Fellowship**_. People should be accountable to one another, not to the script. That means that the script mostly makes suggestions, but is not the source of messages and instructions itself. The script and its author are not an authority, but lend authority to the players.//- _**Media-Independence**_. Ideally, the system should not assume that everything will be done through it's screen-based interfaces. The same script actions which can be fulfilled via text chat shuld also be fulfillable via direct, in-person interactions that are detected by the system, through synchronous or asynchronous voice, or through any other organic means by which the user can carry ou their role.//- _**Adaptation**_. It should be possible for a innovative user to investigate the scripts behind whatever she sees, find out who wrote them, and make edits that run live, in parallel in the same chatroom, that suggest other ways of coordinating.\\\\**Our strategy**\\\\To\\1. Separate out the conversation flow and data collection/distribution part of programming into a script that everyone can understand and modify//2. Present at least two easy to understand representations of the program state, as a thread of chat messages/actions and as a browsable database//3. Limit the script to making suggestions as to what kind of data each user should share or forward around to other users, so that no one has to interact directly with the script according to its expectations//4. Design the script format so that the same script lines can automate interactions across several mediums: text chat, video chat, VR, and most importantly just crossing the room to talk with your colleagues\\\\
