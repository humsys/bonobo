...
function suggestions(thread, script, userId){
  if (!script) return []
  let {roles={}, messages={}} = thread
  let castAsAnyOf = (rs) => rs.some(
    r => r == 'members' || roles[r] && roles[r][userId]
  )
  return script.cues.filter(m => { ///Each notification relates to a cue from the script. Conditions for a particular cue are met if:
    if (Object.values(messages).some(
      ms => ms.suggestionId == `draft-${m.id}-${userId}`
    )) return false ///- if the suggestion wasn't already used
